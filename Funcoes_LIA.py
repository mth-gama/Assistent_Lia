...
def executar_arquivo(maquina,nome):
    ...
    while pause == True:
        ...
        arquivo = ('C:\Python_Projects\Programas_por_Math_Gama\IA LIA\ScriptsDeExecucao/'+caminho)
        if caminho == 'sair':
...
def saber_horas (maquina, nome):
    ...
    teste = int(hora)
    if teste <= 1159:
        maquina.say('Bom dia '+ nome +', agora são exatamente' + hora2+' da manhã')
        maquina.runAndWait()
    elif (teste >= 1200) & (teste <= 1800):
        maquina.say('Boa tarde '+ nome+', agora são exatamente' + hora2+' da tarde')
        maquina.runAndWait()
    elif (teste >= 1900) & (teste <= 100):
        maquina.say('Boa noite '+ nome +', agora são exatamente' + hora2 +' da noite')
        maquina.runAndWait()
...
def contar_piada(maquina):
    ...
    audios = ['Se liga nessa piada',    'Essa vai ser engraçada se liga',  'Mano, essa aqui eu ri pakas', 'Olha essa']
    piadas = [piada01,piada02,piada03,piada04,piada05,piada06,piada07,piada08,piada09,piada10]
    maquina.say(random.choice(audios))
